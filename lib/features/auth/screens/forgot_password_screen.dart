...
class _ForgotPasswordScreenState extends State<ForgotPasswordScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _onContinue() async {
    ...
    if (_selectedMethod == ResetMethod.email) {
      ...
      phone = '$_selectedCountryCode${_phoneCon.text.trim()}';
    }

    setState(() => _isLoading = true);

    final target = email?.isNotEmpty == true ? email! : phone ?? '';

    // Gọi ViewModel để gửi OTP
    await authVM.forgotPassword(target);
    ...
    if (authVM.isSuccess) {
      context.push(
        ...
        extra: {
          'title': title,
          'email': target,
          'actionType': OtpActionType.resetPass,
        },
      );
    }
  }
