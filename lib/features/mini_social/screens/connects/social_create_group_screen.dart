...
class _SocialCreateGroupScreenState extends State<SocialCreateGroupScreen> {
  ...
  Future<void> _onSubmitGroup() async {
    ...
    final mainImageUrl = urls.isNotEmpty ? urls[0] : '';
    final coverImageUrl = urls.length > 1 ? urls[1] : '';
    ...
    final group = SocialGroupsModel(
      ...
      avatarUrl: mainImageUrl,
      memberCount: 0,
      postCount: 0,
      tags: _hashtags,
      createdAt: DateTime.now(),
    );
    final createdGroup = await socialGroupsVm.createGroup(group: group, userId: user.idUser);
    if (!mounted) return;
                  ...
                  GestureDetector(
                    ...
                    child: Stack(
                      children: [
                        ...
                        if (_mainImagePath != null && _mainImagePath!.isNotEmpty)
                          Positioned(
                            top: 8.h,
                            right: 8.w,
                            child: GestureDetector(
                              onTap: () => setState(() => _mainImagePath = null),
                              child: Container(
                                decoration: BoxDecoration(
